...
SET duration_type = CASE 
  WHEN (end_date - start_date) <= INTERVAL '1 day' THEN 'daily'
  WHEN (end_date - start_date) <= INTERVAL '3 days' THEN '3_day'
  ELSE 'weekly'
END,
reset_cost_xp = CASE 
  WHEN (end_date - start_date) <= INTERVAL '1 day' THEN 0  -- Daily challenges reset for free
  WHEN (end_date - start_date) <= INTERVAL '3 days' THEN 100  -- 3-day challenges cost 100 XP
  ELSE 300  -- Weekly challenges cost 300 XP
END,
auto_reset = CASE 
  WHEN (end_date - start_date) <= INTERVAL '1 day' THEN true  -- Daily challenges auto-reset
