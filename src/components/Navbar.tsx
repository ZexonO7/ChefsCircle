...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { name: 'AI recipe', path: '/ingredients-to-recipes', isSpecial: true },
    { name: 'News', path: '/blog' }
  ];
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled 
        ? 'bg-chef-warm-ivory/95 backdrop-blur-lg border-b border-chef-royal-green/20 shadow-chef-luxury' 
        : 'bg-chef-warm-ivory border-b border-chef-royal-green/10'
    }`}>
      <div className="chef-container py-4 flex items-center justify-between">
              ...
              <Link 
                ...
                className={`relative px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 group ${
                  isActivePath(item.path)
                    ? 'text-chef-royal-green bg-chef-royal-green/10'
                    : 'text-chef-charcoal hover:text-chef-royal-green hover:bg-chef-royal-green/5'
                } ${item.isSpecial ? 'bg-gradient-to-r from-chef-royal-blue/10 to-chef-royal-green/10 border border-chef-royal-blue/20' : ''}`}
              >
                {item.isSpecial && (
                  <Sparkles className="w-3 h-3 inline-block mr-1 text-chef-royal-blue animate-pulse" />
                )}
                {item.name}
                {item.isSpecial && (
                  <span className="absolute -top-1 -right-1 w-2 h-2 bg-chef-gold rounded-full animate-ping"></span>
                )}
                <div className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-chef-royal-green transition-all duration-300 group-hover:w-full ${
        ...
        {isSmallScreen && (
          <Button 
            ...
            size="icon" 
            onClick={toggleMenu} 
            className={`text-chef-charcoal hover:bg-chef-royal-green/10 rounded-full transition-all duration-300 ${
              isMenuOpen ? 'bg-chef-royal-green/10 rotate-90' : ''
            }`}
          >
              ...
              <div className="p-6 space-y-2">
                {navItems.map((item, index) => (
                  <Link 
                    key={item.name} 
                    to={item.path} 
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl text-chef-charcoal hover:text-chef-royal-green hover:bg-chef-royal-green/10 transition-all duration-300 transform hover:translate-x-2 ${
                      isActivePath(item.path) ? 'bg-chef-royal-green/10 text-chef-royal-green' : ''
                    } ${item.isSpecial ? 'bg-gradient-to-r from-chef-royal-blue/5 to-chef-royal-green/5 border border-chef-royal-blue/20' : ''}`}
                    onClick={closeMenu}
                    style={{ animationDelay: `${index * 50}ms` }}
                  >
                    {item.isSpecial && (
                      <Sparkles className="w-4 h-4 text-chef-royal-blue animate-pulse" />
                    )}
                    <span className="font-medium">{item.name}</span>
                    {item.isSpecial && (
                      <span className="ml-auto text-xs bg-chef-gold/20 text-chef-gold px-2 py-1 rounded-full">AI</span>
                    )}
                  </Link>
                ))}
              </div>
