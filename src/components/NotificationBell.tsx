...
const NotificationBell = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const { data: regularNotifications, error: notificationsError } = await supabase
        .from('notifications')
        .select('*')
        .eq('user_id', user.id)
    ...
    const markAsSeen = async () => {
      ...
      if (!error) {
        setNotifications((notifs) =>
          notifs.map(n => unseenIds.includes(n.id) ? { ...n, seen: true } : n)
        );
      }
    };
    markAsSeen();
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-96 bg-white shadow-xl rounded-md border border-chef-royal-blue/15 z-50 max-h-96 overflow-hidden">
          <div className="p-4 border-b text-chef-charcoal font-semibold bg-chef-warm-ivory flex items-center justify-between">
            <span>Notifications</span>
            {unseenCount > 0 && (
              <span className="text-xs bg-chef-royal-blue text-white px-2 py-1 rounded-full">
                {unseenCount} new
              </span>
            )}
          </div>
          <div className="max-h-80 overflow-y-auto">
            ...
            {!loading && notifications.length === 0 && (
              <div className="p-4 text-chef-charcoal/60 text-center">
            ...
            {!loading && notifications.slice(0, 15).map(n => (
              <div
                key={n.id}
                className={`flex items-start gap-3 px-4 py-3 border-b last:border-b-0 text-sm transition-all hover:bg-chef-royal-blue/5 ${getNotificationStyle(n.type, n.seen)}`}
              >
                ...
                {!n.seen && n.type !== 'achievement' && (
                  <div className="w-2 h-2 bg-chef-royal-blue rounded-full mt-2 flex-shrink-0"></div>
                )}
              </div>
            ))}
          </div>
          <div className="p-3 border-t bg-chef-warm-ivory/50">
            <button
              className="w-full text-chef-royal-blue text-xs hover:underline bg-transparent"
